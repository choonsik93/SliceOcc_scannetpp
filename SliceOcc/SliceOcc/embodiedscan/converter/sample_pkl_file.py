...
PKL_PATH = "/home/sequor/Downloads/embodiedscan/embodiedscan_infos_train.pkl"  # 경로 조정
OCC_DIR = "/home/sequor/Downloads/embodiedscan/embodiedscan_occupancy"
...
pointcloud_range = [-3.2, -3.2, -0.78, 3.2, 3.2, 1.78]
voxel_dims = [40, 40, 16]
voxel_size = 0.16
...
def print_sample(s, idx):
    ...
    print("occupancy.npy keys: ", occ_data.shape)
    ...
    visible_occ_path = os.path.join(OCC_DIR, s["sample_idx"], "visible_occupancy.pkl")
    visible_occ_data = load_pkl(visible_occ_path)
    print(visible_occ_data[0]["img_path"])
    print(visible_occ_data[0]["visible_occupancy"].shape)

    # Visualize occupancy
    o3d_global_pcd = o3d.geometry.PointCloud()
    o3d_global_pcd.points = o3d.utility.Vector3dVector(occ_data[:, :3].astype(np.float32))
    o3d_global_voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(o3d_global_pcd, voxel_size=1)
    for idx in range(0, 100, 20):
        visible_mask_sample = visible_occ_data[idx]["visible_occupancy"]
        ijk = np.argwhere(visible_mask_sample)
        o3d_pcd = o3d.geometry.PointCloud()
        o3d_pcd.points = o3d.utility.Vector3dVector(ijk.astype(np.float32))
        o3d_pcd.paint_uniform_color([1, 0, 0])
        o3d_voxel_grid = o3d.geometry.VoxelGrid.create_from_point_cloud(o3d_pcd, voxel_size=1)
        o3d.visualization.draw_geometries([o3d_voxel_grid, o3d_global_voxel_grid])
    ...
    for j, im in enumerate(images):
        ...
        visible_instance_ids = im["visible_instance_ids"]
    ...
    o3d.visualization.draw_geometries(o3d_cam_axis_list + [o3d_voxel_grid])
