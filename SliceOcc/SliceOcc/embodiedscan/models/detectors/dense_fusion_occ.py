...
class DenseFusionOccPredictor(BaseModel):
    ...
    def extract_feat(self, batch_inputs_dict: dict,
        ...
        #print(torch.max(points[0][...,2]), torch.min(points[0][...,2]))
        ...
        feat = points_batch[0]
        coor = grid_ind_batch[0]
        

        input_size = coor.shape[0]
        intensity = torch.ones((input_size, 1), device=feat.device, dtype=feat.dtype)
        feats = torch.cat((feat, intensity), dim=1)
        batch = torch.full((input_size, 1), 0, device=feat.device, dtype=torch.int32)
        coords = torch.cat((batch, coor), dim=1)
        pt = PointTensor(feats=feats, coords=coords)
        #st = initial_voxelize(pt, 0.16, 0.16)
        #point_volume = self.backbone_3d(st)

        #point_volume = self.backbone_3d(pt)


        sparse_point_feat = self.backbone_3d(points, grid_ind_batch, 1)
        point_volume = sparse_point_feat
        #sparse_point_feat = self.backbone_3d(sparse_point_feat)
        ...
        #x = sparse_point_feat
        #p2i_fusion_feature = self.slicenet(img_feats_reshaped, batch_img_metas, img_volume, points[0][sampled_indices])
