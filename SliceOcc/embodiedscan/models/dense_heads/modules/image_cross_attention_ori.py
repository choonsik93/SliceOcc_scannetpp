...
class TPVImageCrossAttention(BaseModule):
    ...
    def forward(self,
        ...
        #plane_sizes = [self.tpv_h*self.tpv_w] * self.num_slices[0] * 2 + [2*self.tpv_h*self.tpv_z] * self.num_slices[1] * 2 + [2*self.tpv_w*self.tpv_z] * self.num_slices[2] * 2
        ...
        for tpv_idx, tpv_mask in enumerate(tpv_masks):
            ...
            #print(reference_points_cam)#([20, 1, 320, 3, 2])
            ...
            #queries_rebatch = queries[tpv_idx].new_zeros(
            #    [bs * self.num_cams, max_len * (neighborhood_size**2 + 1), self.embed_dims])
            #reference_points_rebatch = reference_points_cam.new_zeros(
            #    [bs * self.num_cams, max_len * (neighborhood_size**2 + 1), D, 2])
            ...
            for i, reference_points_per_img in enumerate(reference_points_cam):
                for j in range(bs):
                    ...
                    #neighborhood_indices = self.get_neighborhood_indices(index_query_per_img, neighborhood_size, query_spatial_shapes)
                    #queries_rebatch[j * self.num_cams + i, :len(neighborhood_indices)] = queries[tpv_idx][j, neighborhood_indices]
                    #reference_points_rebatch[j * self.num_cams + i, :len(neighborhood_indices)] = reference_points_per_img[j, neighborhood_indices]            
        ...
        value = value.permute(0, 2, 1, 3).view(
            self.num_cams * bs, l, self.embed_dims)
        #print("no1", len(queries_rebatches))
        ...
        for tpv_idx, indexes in enumerate(indexeses):
            for i, index_query_per_img in enumerate(indexes):
                for j in range(bs):
                    neighborhood_indices = self.get_neighborhood_indices(index_query_per_img, neighborhood_size, query_spatial_shapes)
                    for k, idx in enumerate(index_query_per_img):
                        #start_n = k * (neighborhood_size + 1)**2
                        #end_n = (k+1) * (neighborhood_size + 1)**2
                        start_n = k * (neighborhood_size**2 + 1)
                        end_n = (k+1) * (neighborhood_size**2 + 1)                        
                        slots[tpv_idx][j, k] += queries[tpv_idx][j * self.num_cams + i, start_n: end_n].mean(dim=0)
        '''


    def get_neighborhood_indices(self, indices, neighborhood_size, query_spatial_shape):
        """
        Get indices of the neighborhood around each index in `indices`.
        Args:
            indices (Tensor): The indices of the query points.
            neighborhood_size (int): The size of the neighborhood.
            query_spatial_shape (Tuple[int, int]): The spatial shape of the query feature map (height, width).
        Returns:
            Tensor: The indices of the neighborhood.
        """
        ...
        for idx in indices:
            ...
            neighborhood_indices.append(idx.item())
            for i in range(-half_size, half_size + 1):
                for j in range(-half_size, half_size + 1):
                    new_row = row + i
                    new_col = col + j
                    if (i!=0 and j!=0) and 0 <= new_row < h and 0 <= new_col < w:
                        new_index = new_row * w + new_col
                        neighborhood_indices.append(new_index)

        return torch.tensor(neighborhood_indices, device=indices.device, dtype=indices.dtype)
