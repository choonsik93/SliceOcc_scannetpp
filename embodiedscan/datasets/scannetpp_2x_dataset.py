...
#from embodiedscan.datasets.transforms import *


@DATASETS.register_module()
class Scannetpp2xDataset(BaseDataset):
    """ScanNet++-like dataset built directly on BaseDataset.

    This class mirrors EmbodiedScanDataset behavior with minimal changes:
      - Loads a pickle annotation file (list of 'info' dicts).
      - For each sample, parses 3D boxes/labels, occupancy, and visible masks.
      - Custom path resolver for occupancy/masks when sample_idx starts with 'scannetpp'.
      - Provides a safe default occ_label_mapping if none is given.

    Expected annotation 'info' schema (same as original):
      info = {
        'sample_idx': 'scannetpp/<region>' or other supported prefixes,
        'images': [  # multi-view
          {
            'img_path': '.../image_xxx.jpg',
            # optional: 'visible_instance_ids': [int,...]
          },
          ...
        ],
        'instances': [
          {
            'bbox_3d': [x, y, z, dx, dy, dz, roll, pitch, yaw],
            'bbox_label_3d': raw_label_int
          },
          ...
        ],
    ...
    def __init__(self,
        ...
        super().__init__(ann_file=ann_file,
                         ...
                         test_mode=test_mode,
                         **kwargs)
        self.occ_distribution = np.zeros((16, 81))
    ...
    def parse_ann_info(self, info: dict) -> dict:
        ...
        '''
        shaped_occ_gt = occ_multiscale_supervision(gt_occ, 1,
                                        (1,81,40,40,16),
                                        None)
        ...
        #     print(occ_masks[i]['visible_occupancy'].shape)
        #     ann_info['visible_occupancy_masks'].append(
        #         occ_masks[i]['visible_occupancy'])

        # Debug
        # grid = tuple(self.metainfo.get('occ_grid_shape', (512, 512, 80)))
        # ann_info['visible_occupancy_masks'] = []
        # for i in range(len(info['images'])):
        #     ann_info['visible_occupancy_masks'].append(
    ...
    @staticmethod
    def compute_height_distribution(labels, num_classes=81, height_dim=16):

        height_distribution = np.zeros((height_dim, num_classes))
        for z in range(height_dim):
            layer_labels = labels[:, :, z]
            for label in range(1, num_classes+1):
                count = np.sum(layer_labels == label)
                height_distribution[z, label] = count
