...
def generate_scannetpp_info(data_root_dir, scene_id_list, split="train"):
    ...
    for scene_id in tqdm(scene_id_list):
        ...
        occupancy_path = os.path.join(data_root_dir, "occupancy_2x", scene_id, "occupancy.npy")
        axis_align_matrix_path = os.path.join(data_root_dir, "occupancy_2x", scene_id, "axis_align_matrix.npy")
        if os.path.exists(occupancy_path) and os.path.exists(axis_align_matrix_path):
            AssertionError(f"Occupancy data not exists for scene {scene_id} in split {split}.")
...
train_info_save_path = os.path.join(data_root_dir, f"scannetpp_infos_2x_train.pkl")
...
val_info_save_path = os.path.join(data_root_dir, f"scannetpp_infos_2x_val.pkl")
