...
class TwoStageKMeans:
    def __init__(self, initial_clusters=50, z_clusters=3, tolerance=0.001, max_iter=50):
        self.initial_clusters = initial_clusters
        self.z_clusters = z_clusters
        self.tolerance = tolerance
        self.max_iter = max_iter
        self.initial_kmeans = KMeans(n_clusters=initial_clusters, max_iter=max_iter)
        self.z_kmeans = KMeans(n_clusters=z_clusters, max_iter=max_iter)
        self.initial_centers = None
        self.z_cluster_centers = None
        self.z_labels = None
    
    def fit(self, point_cloud):
        #print(point_cloud)
        self.initial_kmeans.fit(point_cloud)
        self.initial_centers = self.initial_kmeans.cluster_centers_
        z_values = self.initial_centers[:, 2].reshape(-1, 1)

        z_values = np.array([[0.0] if (math.isinf(x[0]) or math.isnan(x[0])) else x for x in z_values]).reshape(-1, 1)
        self.z_kmeans.fit(z_values)
        self.z_cluster_centers = self.z_kmeans.cluster_centers_
        self.z_labels = self.z_kmeans.labels_

    def get_initial_centers(self):
        return self.initial_centers

    def get_z_cluster_centers(self):
        return self.z_cluster_centers

    def get_z_labels(self):
        return self.z_labels


    def get_z_height_segments(self, lower_bound, upper_bound):
        if self.z_cluster_centers is None:
            return []
        
        sorted_z_centers = sorted(self.z_cluster_centers.flatten())
    
        z_segments = []
        num_segments = len(sorted_z_centers)
        
        for i in range(num_segments):
            if i == 0:
                lower = min(lower_bound, sorted_z_centers[i] - (sorted_z_centers[i+1] - sorted_z_centers[i]) / 2)
            else:
                lower = (sorted_z_centers[i-1] + sorted_z_centers[i]) / 2
            
            if i == num_segments - 1:
                upper = max(upper_bound, sorted_z_centers[i] + (sorted_z_centers[i] - sorted_z_centers[i-1]) / 2)
            else:
                upper = (sorted_z_centers[i] + sorted_z_centers[i+1]) / 2
            
            z_segments.append((lower, upper))
    ...
    def visualize(self, point_cloud):
        ...
        ax.scatter(point_cloud[:, 0], point_cloud[:, 1], point_cloud[:, 2], c='blue', marker='o')
        
        for z_label in np.unique(self.z_labels):
            indices = np.where(self.z_labels == z_label)
            centers_in_z_cluster = self.initial_centers[indices]
            ax.scatter(centers_in_z_cluster[:, 0], centers_in_z_cluster[:, 1], centers_in_z_cluster[:, 2], s=100, label=f'Z-cluster {z_label}')
        

        z_segments = self.get_z_height_segments()
        for lower, upper in z_segments:
            ax.plot([0, 1], [0, 1], zs=[lower, upper], color='red')
...
class K_Means:
    ...
    def get_cluster_radii(self):
        if not self.fitted:
            print("Unfitted.")
            return []
        radii = np.zeros(self.k_)
        for i in range(self.k_):
            points = np.array(self.labels_[i])
            if points.size == 0:
                continue
            distances = np.linalg.norm(points - self.centers_[i], axis=1)
...
if __name__ == "__main__":
    ...
    point_cloud_array = np.random.rand(1000, 3).astype(np.float32)

    two_stage_kmeans = TwoStageKMeans(initial_clusters=10, z_clusters=3)
    two_stage_kmeans.fit(point_cloud_array)

    initial_centers = two_stage_kmeans.get_initial_centers()
    z_cluster_centers = two_stage_kmeans.get_z_cluster_centers()
    z_labels = two_stage_kmeans.get_z_labels()
    heights = two_stage_kmeans.get_z_height_segments(0.0, 1.0)

    print("Initial Cluster Centers:")
    print(initial_centers)
    print("Z-axis Cluster Centers:")
    print(z_cluster_centers)
    print("Z-axis Clustered Heights:")
    print(heights)

    #two_stage_kmeans.visualize(point_cloud_array)