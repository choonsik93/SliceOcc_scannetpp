...
from ..layers.fusion_layers.point_fusion import batch_point_sample, batch_point_sample_with_label
...
class EmbodiedOccPredictor(BaseModel):
    ...
    def extract_feat(self, batch_inputs_dict: dict,
        ...
        #print(len(points)) #num_cams
        filter_points = []
        for p in points:
            if p.size(0) == 10000:
                filter_points.append(p)

        img_points = torch.stack(filter_points, dim=0).squeeze(0) #points[0] 

        #print(img_points.shape) torch.Size([10000, 3])
        volumes, valid_preds = [], []
        for idx in range(len(batch_img_metas)):
            ...
            if isinstance(proj_mat, dict):
                ...
                grid_size_x = (3.2 - (-3.2)) / 40
                grid_size_y = (3.2 - (-3.2)) / 40
                grid_size_z = (1.28 - (-1.28)) / 16

                indices_x = torch.clamp(((prior_points[:, 0] - (-3.2)) / grid_size_x).long(), 0, 39)
                indices_y = torch.clamp(((prior_points[:, 1] - (-3.2)) / grid_size_y).long(), 0, 39)
                indices_z = torch.clamp(((prior_points[:, 2] - (-1.28)) / grid_size_z).long(), 0, 15)

                gt_occ = [
                    data_samples.gt_occupancy for data_samples in batch_data_samples
                ][0]

                gt = torch.zeros([1, 40, 40, 16]).to(gt_occ.device).type(torch.long)       
                coords = torch.div(gt_occ[:, :3].type(torch.long), 1, rounding_mode='trunc')
                gt[:, coords[:, 0], coords[:, 1], coords[:, 2]] = gt_occ[:, 3]
                #print(gt.shape)
                point_labels = gt[:, indices_x, indices_y, indices_z]

                _2d_label = batch_point_sample_with_label(
                    img_meta,
                    points=img_points,
